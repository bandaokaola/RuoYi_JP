...
function createMenuItem(dataUrl, menuName, isRefresh) {
    ...
    if (flag) {
        ...
        window.parent.$.modal.loading("データを読み込んでいます。お待ちください...");
...
function checkpwd(chrtype, password) {
    if (chrtype == 1) {
        if (!$.common.numValid(password)) {
            $.modal.alertWarning("パスワードは0から9までの数字のみです。");
            return false;
        }
    } else if (chrtype == 2) {
        if (!$.common.enValid(password)) {
            $.modal.alertWarning("パスワードはaからzおよびAからZのアルファベットのみです。");
            return false;
        }
    } else if (chrtype == 3) {
        if (!$.common.enNumValid(password)) {
            $.modal.alertWarning("パスワードには文字と数字が含まれる必要があります。");
            return false;
        }
    } else if (chrtype == 4) {
        if (!$.common.charValid(password)) {
            $.modal.alertWarning("パスワードには文字、数字、および特殊記号が必要です。<font color='red'>~!@#$%^&*()-=_+</font>");
            return false;
        }
    }
    return true;
...
$.ajaxSetup({
    complete: function(XMLHttpRequest, textStatus) {
        if (textStatus == 'timeout') {
            $.modal.alertWarning("サーバータイムアウト、後でもう一度お試しください。");
            $.modal.enable();
            $.modal.closeLoading();
        } else if (textStatus == "parsererror" || textStatus == "error") {
            $.modal.alertWarning("サーバーエラーが発生しました。管理者に連絡してください。");
            $.modal.enable();
            $.modal.closeLoading();
        }
    }
